...
> Estado al cerrar la sesión. Contexto: el chat se movió del detalle de reserva a una vista
> propia (`/messages`, dos paneles) y se cableó el handshake autenticado + join a room contra
> el worker. La UI y el transporte funcionan; lo que falta es la **capa de datos del hilo** y
> resolver una duplicación de regla entre repos.
